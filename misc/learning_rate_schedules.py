...
class CosineDecayLW(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, step=2000):
        ...
        cosine_decayed = 0.5 * (1.0 + tf.cos(
            tf.constant(math.pi, dtype=dtype) * global_step_recomp / decay_steps))
        decayed = (1 - self.alpha) * cosine_decayed + self.alpha
        #decayed_lr = tf.multiply(initial_learning_rate, decayed)

        # less (<) is correct because the step will start at zero.
...
if __name__ == "__main__":
    decay_schedule = CosineDecayLW(start_lr=0.00001, lower_bound_lr=0, upper_bound_lr=0.00005,
                                   warmup_steps=4999, decay_steps=45000)

    print([decay_schedule(i).numpy() for i in range(30000)])

    fig = plt.figure()
    plt.plot([decay_schedule(i).numpy() for i in range(30000)])
    plt.show()
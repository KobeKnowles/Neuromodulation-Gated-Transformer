...
if __name__ == "__main__":
    ...
    num_aux_toks = 2
    config_dict["max_seq_len"] = 510 # not 512 b/c of the two auxiliary tokens; we can't change the max_position_embeddings,
    # it raises an error.
    ...
    if strategy is not None:
        with strategy.scope():
            config = BertConfig(vocab_size=config_dict["vocab_size"], hidden_size=config_dict["hidden_size"],
                                ...
                                num_aux_toks=num_aux_toks, max_seq_len=3)
    ...
    dloader = general_data_loader(filepath_BoolQ="/large_data/SuperGlue/BoolQ/train.jsonl",
                                  ...
                                  is_test_MPE=is_test,
                                  is_BoolQ=True, is_CB=True, is_COPA=True, is_MultiRC=False, is_ReCoRD=False, is_MPE=False,
                                  is_RTE=False, is_WiC=True, is_WSC=False, is_CQA=False, is_RACE=False, is_SciTail=False,
    ...
    data = dloader.get_generalQA_generator(batch_size, head_strategy="uniform", dataset_strategy="uniform",
                                           shuffle_datasets=shuffle_datasets, type="ablation",
                                           num_iterations=num_iterations, is_aux_toks=True)
    if strategy is not None:
        data = strategy.experimental_distribute_dataset(data)

    train_save_filepath = "/data/kkno604/NGT_experiments_updated/aux_tok_ablation/gating-end-aux-toks/exp3/Results/"
    checkpoint_save_filepath = "/data/kkno604/NGT_experiments_updated/aux_tok_ablation/gating-end-aux-toks/exp3/Checkpoints/"
    ...
    train_class.training_iteration(iterations=num_iterations, dataLoader=data,
                                   ...
                                   batch_size=batch_size, save_iterations=[10000, 20000, 30000],

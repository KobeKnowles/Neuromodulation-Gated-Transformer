...
def COPA_return_correct_incorrect_ids(pred_file, answer_file="/large_data/SuperGlue/COPA/val.jsonl"):
    ...
    for exp in range(1, 4):
        for iteration in [10000, 20000, 30000]:
            for type_ in ["gating-end-aux-toks", "no-gating-end-aux-toks",
                ...
                #            "get_results_in_domain/COPA/prediction_files/iteration"+str(iteration)+".jsonl"

                pred_file = "/data/kkno604/NGT_experiments_updated/aux_tok_ablation/"+type_+"/" \
                            "exp"+str(exp)+"/Results/results_COPA/prediction_files/iteration"+str(iteration)+".jsonl"

                answer_file = "/large_data/SuperGlue/COPA/val.jsonl"

                #save_filepath = "/data/kkno604/NGT_experiments_updated/general_experiments/"+type_+"/exp"+str(exp)+"/Results/" \
                #                "get_results_in_domain/COPA/Accuracy/iteration"+str(iteration)+".jsonl"

                save_filepath = "/data/kkno604/NGT_experiments_updated/aux_tok_ablation/"+type_+"/" \
                                "exp"+str(exp)+"/Results/results_COPA/Accuracy/iteration"+str(iteration)+".jsonl"

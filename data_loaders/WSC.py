...
class WSCLoader:
    ...
    def __init__(self, filepath, tokenizer, is_test=False, shuffle=False, max_seq_len=512, is_token_type_ids: bool=True,
        ...
        for example in self.data:
            ...
            self.target_span1.append(example["target"]["span1_text"])
            self.target_span2.append(example["target"]["span2_text"])
        ...
        assert len(self.text) == len(self.target_span1)
        assert len(self.target_span2) == len(self.target_span1)
        if not is_test: assert len(self.text) == len(self.label)
    ...
    def _process_data(self):
        ...
        for i, txt in enumerate(self.text):
            ...
            tmp_dict = {}
            tmp_dict["text"] = txt
            tmp_dict["span1"] = self.target_span1[i]
            tmp_dict["span2"] = self.target_span2[i]
    ...
    def __call__(self, batch_size):
        ...
        for i, example in enumerate(self.processed_data):
            ...
            sentence_a, sentence_b = example["text"], example["span1"] + " [SEP] " + example["span2"]
...
if __name__ == "__main__":
    ...
    dataLoader = WSCLoader(filepath="/large_data/SuperGlue/WSC/train.jsonl",
                           tokenizer=tokenizer,
                           is_test=is_test)

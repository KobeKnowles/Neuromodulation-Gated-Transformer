...
class WiCLoader:
    ...
    def __init__(self, filepath, tokenizer, is_test=False, shuffle=False, max_seq_len=512, is_token_type_ids: bool=True,
        ...
        for example in self.data:
            if ids_to_exclude is not None:
                if example["idx"] in ids_to_exclude:
                    exclusion_counter += 1
                    continue
            self.word.append(example["word"])
            self.sentence1.append(example["sentence1"])
            self.sentence2.append(example["sentence2"])
        ...
        assert len(self.word) == len(self.sentence1)
        assert len(self.sentence2) == len(self.sentence1)
        if not is_test: assert len(self.sentence1) == len(self.label)
    ...
    def _process_data(self):
        ...
        for i, word in enumerate(self.word):
            ...
            tmp_dict = {}
            tmp_dict["word"] = word
            tmp_dict["sentence1"] = self.sentence1[i]
            tmp_dict["sentence2"] = self.sentence2[i]
    ...
    def _label_to_int(self, label: bool):
        if label: return [1] #if True return 1
        elif label is False: return [0] #if False return 0
    ...
    def __call__(self, batch_size):
        ...
        for i, example in enumerate(self.processed_data):
            ...
            # str_ = "[CLS] " + self.sentence1[i] + " [SEP] " + self.sentence2[i] + " [SEP]" + self.word[i] + [SEP]
            sentence_a, sentence_b = example["sentence1"] + " [SEP] " + example["sentence2"],  example["word"]
...
if __name__ == "__main__":
    ...
    dataLoader = WiCLoader(filepath="/large_data/SuperGlue/WiC/train.jsonl",
                           tokenizer=tokenizer,
                           is_test=is_test)
